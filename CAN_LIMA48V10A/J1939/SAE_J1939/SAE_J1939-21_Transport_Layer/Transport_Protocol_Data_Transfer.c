...
void SAE_J1939_Read_Transport_Protocol_Data_Transfer(J1939 *j1939, uint8_t SA, uint8_t data[]) {
	...
	PGN = j1939->from_other_ecu_tp_cm.PGN_of_the_packeted_message;
	total_message_size = j1939->from_other_ecu_tp_cm.total_message_size_being_transmitted;
	inserted_bytes = 0;
...
ENUM_J1939_STATUS_CODES SAE_J1939_Send_Transport_Protocol_Data_Transfer(J1939 *j1939, uint8_t DA){
	...
	case CONTROL_BYTE_TP_CM_BAM:
		for (i = 1; i <= j1939->this_ecu_tp_cm.number_of_packages_beging_transmitted; i++) {
			...
			CAN_Delay(10);																		/* Important CAN delay according to standard */
