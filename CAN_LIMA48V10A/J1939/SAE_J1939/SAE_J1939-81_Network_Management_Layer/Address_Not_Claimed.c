...
void SAE_J1939_Read_Address_Not_Claimed(J1939 *j1939, uint8_t SA, uint8_t data[]) {
	j1939->from_other_ecu_name.identity_number = ((data[2] & 0x1F) << 16) | (data[1] << 8) | data[0];
	j1939->from_other_ecu_name.manufacturer_code = (data[3] << 3) | (data[2] >> 5);
	j1939->from_other_ecu_name.function_instance = data[4] >> 3;
	j1939->from_other_ecu_name.ECU_instance = data[4] & 7;
	...
	j1939->from_other_ecu_name.industry_group = (data[7] >> 4) & 7;
	j1939->from_other_ecu_name.vehicle_system_instance = data[7] & 0x0F;
