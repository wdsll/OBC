...
ENUM_J1939_STATUS_CODES SAE_J1939_Response_Request_DM1(J1939* j1939, uint8_t DA) {
	...
	if(j1939->this_dm.errors_dm1_active < 2) {
		...
		data[4] = ((j1939->this_dm.dm1.SPN[0] >> 11) & 0xE0) | j1939->this_dm.dm1.FMI[0];
		...
		for (i = 0; i < j1939->this_ecu_tp_cm.total_message_size_being_transmitted; i++){
			...
			j1939->this_ecu_tp_dt.data[i*4 + 4] = ((j1939->this_dm.dm1.SPN[i] >> 11) & 0xE0) | j1939->this_dm.dm1.FMI[i];
...
void SAE_J1939_Read_Response_Request_DM1(J1939 *j1939, uint8_t SA, uint8_t data[], uint8_t errors_dm1_active) {
	...
	j1939->from_other_ecu_dm.dm1.SAE_lamp_status_red_stop = (data[0] >> 4) & 0x03;
	j1939->from_other_ecu_dm.dm1.SAE_lamp_status_amber_warning = (data[0] >> 2) & 0x03;
	j1939->from_other_ecu_dm.dm1.SAE_lamp_status_protect_lamp = data[0] & 0x03;
	j1939->from_other_ecu_dm.dm1.SAE_flash_lamp_malfunction_indicator = data[1] >> 6;
	j1939->from_other_ecu_dm.dm1.SAE_flash_lamp_red_stop = (data[1] >> 4) & 0x03;
	j1939->from_other_ecu_dm.dm1.SAE_flash_lamp_amber_warning = (data[1] >> 2) & 0x03;
	j1939->from_other_ecu_dm.dm1.SAE_flash_lamp_protect_lamp = data[1] & 0x03;

	/* Read and Decode DTC info for up to MAX_DM_FIELD active DTCs */

	for (i = 0; i < errors_dm1_active && i < MAX_DM_FIELD; i++){
		j1939->from_other_ecu_dm.dm1.SPN[i] = ((data[(i*4)+4] & 0xE0) << 11) | (data[(i*4)+3] << 8) | data[(i*4)+2];
		j1939->from_other_ecu_dm.dm1.FMI[i] = data[(i*4)+4] & 0x1F;
		j1939->from_other_ecu_dm.dm1.SPN_conversion_method[i] = data[(i*4)+5] >> 7;
		j1939->from_other_ecu_dm.dm1.occurrence_count[i] = data[(i*4)+5] & 0x7F;
