...
ENUM_J1939_STATUS_CODES SAE_J1939_Send_Request_DM14(J1939 *j1939, uint8_t DA, uint16_t number_of_requested_bytes, uint8_t pointer_type, uint8_t command, uint32_t pointer, uint8_t pointer_extension, uint16_t key) {
	...
	data[1] = ((number_of_requested_bytes >> 3)&0xE0) | (pointer_type << 4) | (command << 1) | 1;
	data[2] = pointer;
	data[3] = pointer >> 8;
	data[4] = pointer >> 16;
...
ENUM_J1939_STATUS_CODES SAE_J1939_Read_Request_DM14(J1939 *j1939, uint8_t DA, uint8_t data[]) {
	...
	uint16_t number_of_requested_bytes = ((data[1] & 0xE0) << 3) | data[0];
	uint8_t pointer_type = (data[1] >> 4) & 1;
	uint8_t command = (data[1] >> 1) & 0x07;
	...
	status = SAE_J1939_Send_Response_DM15(j1939, DA, number_of_allowed_bytes, command, EDC_parameter, EDCP_extention, seed);
