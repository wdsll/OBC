...
ENUM_J1939_RX_MSG Open_SAE_J1939_Listen_For_Messages(J1939* j1939) {
	...
	if(is_new_message) {
		...
		id0 = ID >> 24;
		id1 = ID >> 16;
		DA = ID >> 8; 	/* Destination address which is this ECU. if DA = 0xFF = broadcast to all ECU. Sometimes DA can be an ID number too */
		SA = ID; 		/* Source address of the ECU that we got the message from */
