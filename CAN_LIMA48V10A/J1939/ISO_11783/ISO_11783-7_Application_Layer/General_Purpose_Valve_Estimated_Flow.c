...
ENUM_J1939_STATUS_CODES ISO_11783_Response_Request_General_Purpose_Valve_Estimated_Flow(J1939 *j1939, uint8_t DA) {
	...
	data[2] = (j1939->this_general_purpose_valve_estimated_flow.fail_safe_mode << 6) | (3 << 4) | j1939->this_general_purpose_valve_estimated_flow.valve_state; 	/* Bit 5 and 6 are reserved for further use */
...
void ISO_11783_Read_Response_Request_General_Purpose_Valve_Estimated_Flow(J1939 *j1939, uint8_t SA, uint8_t data[]) {
	...
	j1939->from_other_ecu_general_purpose_valve_estimated_flow.valve_state = data[2] & 0x0F;
